...
prev_month as (
    select
        *,
        ...
        lag(total_active_listings) over (partition by listing_neighbourhood order by month_year) as prev_month_active_listings,
        lag(total_listings - total_active_listings) over (partition by listing_neighbourhood order by month_year) as prev_month_inactive_listings
    from
        aggregation
)
